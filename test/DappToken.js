...
contract("DappToken", function (accounts) {
  ...
  it("transfers token ownership", function () {
    return DappToken.deployed()
      .then(function (instance) {
        tokenInstance = instance;
        //call function does not trigger transaction
        return tokenInstance.transfer.call(accounts[1], 999999999999999999);
      })
      .then(assert.fail)
      .catch(function (error) {
        assert(
          error.message.indexOf("invalid number value") >= 0,
          "error message must contain the message: invalid number value"
        );
    ...
    return DappToken.deployed()
      ...
      .catch(function (error) {
        assert(
          ...
          "cannot transfer valuer larger than balance"
        );
